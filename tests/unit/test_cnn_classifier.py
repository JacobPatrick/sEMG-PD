...
def test_cnn_multi_task_classification():
    ...
    X = np.random.rand(n_samples, n_channels * n_features, n_windows).astype(np.float32)
    ...
    # 断言检查
    assert len(models) == n_labels, f"应该创建 {n_labels} 个模型，但实际创建了 {len(models)} 个"
    
    # 检查第一个模型（二分类任务）
    assert not isinstance(models[0], FixedOutputCNN), "第一个模型不应该是FixedOutputCNN"
    
    # 检查第二个模型（三分类任务）
    assert not isinstance(models[1], FixedOutputCNN), "第二个模型不应该是FixedOutputCNN"
    
    # 检查第三个模型（单类别任务）
    assert isinstance(models[2], FixedOutputCNN), "第三个模型应该是FixedOutputCNN"
    assert models[2].fixed_output == 0, "第三个模型的固定输出应该是0"
    
    # 使用模型进行预测
    X_test = np.random.rand(10, n_channels * n_features, n_windows).astype(np.float32)
...
def test_single_label_classification():
    ...
    assert not isinstance(models[0], FixedOutputCNN), "模型不应该是FixedOutputCNN"
    
    # 使用模型进行预测
    X_test = np.random.rand(5, n_channels * n_features, n_windows).astype(np.float32)
