...
def select_features(
    features_data: Dict[str, Any], feature_set_config: Dict[str, Any]
) -> Dict[str, Any]:
    """根据配置选择特征子集
    ...
    for subject_id, subject_data in features_data["features"].items():
        ...
        for exp_name in feature_set_config["experiments"]:
            ...
            selected[subject_id][exp_name] = {"windows": {}}
            for window_id, window_data in subject_data[exp_name][
                "windows"
            ].items():
...
def prepare_feature_sets(
    features_data: Dict[str, Any], ablation_config: Dict[str, Any]
) -> Dict[str, Any]:
    """准备不同的特征集合"""
    feature_sets = {}
