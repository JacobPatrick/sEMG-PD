...
class TremorParamExtractor:
    ...
    def plotPsd(self, fig_path):
        ...
        """
        _, axes = plt.subplots(
            nrows=self.n_channels,
            sharex=True,
            squeeze=False,
            figsize=(20, self.n_channels * 1.5),
        )
    ...
    def getPowerRatio(self, env_psd_freq, window_size=20000, overlap=10000):
        ...
        for ch in range(self.n_channels):
            # 分段处理信号
            segments = self._segment_signal(
                self.data[:, ch], window_size, overlap
            )
            ...
            for seg in segments:
                ...
                mask = (freqs >= (peak_freq - 0.25)) & (
                    freqs <= (peak_freq + 0.25)
                )
