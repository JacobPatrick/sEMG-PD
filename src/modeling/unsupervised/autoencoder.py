...
class Autoencoder(nn.Module):
    ...
    def __init__(self, pre_data, post_data):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(pre_data.shape[1], 4), nn.ReLU(), nn.Linear(4, 2)
        )
        # 解码器
        ...
        # 将数据转换为 PyTorch 张量
        self.pre_data_tensor = torch.tensor(
            self.pre_data_scaled, dtype=torch.float32
        )
        self.post_data_tensor = torch.tensor(
            self.post_data_scaled, dtype=torch.float32
        )

        # 创建数据集和数据加载器
        pre_dataset = TensorDataset(self.pre_data_tensor)
        self.pre_dataloader = DataLoader(
            pre_dataset, batch_size=32, shuffle=True
        )
    ...
    def train(self, n_epochs=50, learning_rate=0.001):
        ...
        for epoch in range(n_epochs):
            ...
            print(
                f'Epoch {epoch + 1}/{n_epochs}, Loss: {running_loss / len(self.pre_dataloader)}'
            )
    ...
    def compare(self):
        ...
        average_distance = np.mean(
            [
                euclidean(pre_features[i], post_features[i])
    ...
    def visualize(self):
        if self.pre_features is None or self.post_features is None:
            raise ValueError(
                "特征向量为空值，请先调用 compare() 方法进行特征提取和比较。"
            )
            return
        plt.scatter(
            self.pre_features[:, 0],
        ...
        plt.scatter(
            self.post_features[:, 0],
            self.post_features[:, 1],
