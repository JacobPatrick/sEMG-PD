...
def _fit_biases(
    ...
    for dilation_index in range(num_dilations):
        ...
        for kernel_index in range(num_kernels):
            ...
            num_channels_end = (
                num_channels_start + num_channels_this_combination
            )
            ...
            C_gamma = np.zeros(
                (9, num_channels_this_combination, input_length),
                dtype=np.float32,
            )
            C_gamma[9 // 2] = G
...
def _fit_dilations(input_length, num_features, max_dilations_per_kernel):
    ...
    dilations, num_features_per_dilation = np.unique(
        np.logspace(
            0, max_exponent, true_max_dilations_per_kernel, base=2
        ).astype(np.int32),
        return_counts=True,
    )
...
def fit(X, num_features=10_000, max_dilations_per_kernel=32):
    ...
    channel_indices = np.zeros(
        num_channels_per_combination.sum(), dtype=np.int32
    )
...
def transform(X, parameters):
    ...
    for example_index in prange(num_examples):
        ...
        for dilation_index in range(num_dilations):
            ...
            C_gamma = np.zeros(
                (9, num_channels, input_length), dtype=np.float32
            )
            C_gamma[9 // 2] = G
            ...
            for kernel_index in range(num_kernels):
                ...
                feature_index_end = (
                    feature_index_start + num_features_this_dilation
                )
