...
class ManualFeatureExtractor(FeatureExtractor):
    ...
    def extract(self, data: Dict[str, Any], config: ExperimentConfig) -> Dict[str, Any]:
        """实现手工特征提取逻辑
        ...
        overlap = config.feature.overlap
        ...
        for subject_id, subject_data in data["raw"].items():
            ...
            for exp_name, exp_data in subject_data.items():
                ...
                if not isinstance(exp_data, pd.DataFrame):
                    raise ValueError(f"Experiment data for {subject_id}/{exp_name} must be a DataFrame")
                ...
                exp_features = {
                    ...
                    "metadata": {
                        ...
                        "n_features": len(self.time_domain_features) + len(self.freq_domain_features)
                    }
                }
                ...
                for start in range(0, n_samples - window_samples + 1, stride):
                    ...
                    for channel in signals.columns:
                        ...
                        window_features[channel] = self._extract_window_features(
                            window_data, sampling_rate
                        )
                    
                    exp_features["windows"][f"window_{window_id}"] = window_features
                    window_id += 1
    ...
    def _extract_window_features(self, window_data: np.ndarray, sampling_rate: int) -> Dict[str, float]:
        """提取单个窗口的所有特征"""
        features = {}
