...
class SignalPlotter:
    def __init__(self, signal, fs=2000, random=False, **kwargs):
        ...
        if self.random:
            ...
            self.start = np.random.randint(0, self.signal.shape[0] - self.duration * self.fs)
        ...
        self.ylabel = ['ECR-L', 'FCR-L', 'ECR-R', 'FCR-R', 'TA-L', 'GA-L', 'TA-R', 'GA-R']

    def plot_time_domain(self, fig_path):
        _, axes = plt.subplots(nrows=self.channel_num, sharex=True, squeeze=False, figsize=(20, self.channel_num * 1.5))
        x = np.arange(self.sample_num) / self.fs
        for i in range(self.channel_num):
            axes[i, 0].plot(x[self.start:self.end], self.signal[self.start:self.end, i])
