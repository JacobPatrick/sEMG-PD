...
deprecated: use pre_process.py instead
"""
...
class PreProcessor:
    ...
    def filter_emg(
        ...
        for i in range(self.model['emg'].shape[1]):
            self.model['emg'][:, i] = filtfilt(
                b_butter, a_butter, self.model['emg'][:, i]
            )
            self.model['emg'][:, i] = filtfilt(
                b_notch, a_notch, self.model['emg'][:, i]
            )
    ...
    def filter_acc(self, fs=200, highcut=50.0):
        ...
        for i in range(self.model['acc'].shape[1]):
            self.model['acc'][:, i] = filtfilt(
                b_butter, a_butter, self.model['acc'][:, i]
            )
    ...
    def segment_emg(self, window_size=200, overlap=0.5):
        ...
        segments = []
        for i in range(
            0, self.model['emg'].shape[0] - window_size + 1, step_size
        ):
    ...
    def sample_emg(self, fs=2000, duration=5, num_samples=6):
        ...
        for _ in range(num_samples):
            start = np.random.randint(
                0, self.model['emg'].shape[0] - duration * fs
            )
